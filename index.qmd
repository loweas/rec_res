...
```{r, message=FALSE}
library(readr)
welfare_2019_zip<- read_csv("~/Downloads/welfare_2019_zip (1).csv")
welfare_2018_zip <- read_csv("~/Downloads/welfare_2018_zip (1).csv")
#welfare_2018_zip$site=NULL
#welfare_2018_zip$id=NULL
welfare_2020_zip <- read_csv("~/Downloads/welfare_2020_zip (1).csv")
welfare_2021_zip <- read_csv("~/Downloads/welfare_2021_zip (1).csv")
welfare_2022_zip <- read_csv("~/Downloads/welfare_2022_zip (1).csv")
welfare_2023_zip <- read_csv("~/Downloads/welfare_2023_zip (1).csv")
#average_welfare_loss_per_zip <- read_csv("~/Downloads/average_welfare_loss_per_zip.csv")
welfare_2018_zip=welfare_2018_zip %>% group_by(zip) %>% summarise(mean_value=mean(mean_value))


welfare_2018_zip$Year=2018
welfare_2019_zip$Year=2019
welfare_2020_zip$Year=2020
welfare_2021_zip$Year=2021
welfare_2022_zip$Year=2022
welfare_2023_zip$Year=2023

average_welfare_loss_per_zip =rbind(welfare_2018_zip,welfare_2019_zip,welfare_2020_zip,welfare_2021_zip,welfare_2022_zip,welfare_2023_zip)
...
```{r}
ggplot(t, aes(x = island, y = mean_value, fill = factor(CEJST))) +
...
```{r}
ggplot(t, aes(x = as.factor(Year), y = mean_value, fill = as.factor(DAC_flag))) +
    geom_boxplot(outlier.size = 0.5, alpha = 0.8) +
    scale_fill_manual(values = c("0" = "white", "1" = "grey"), 
                      labels = c("Non-DAC", "DAC"), name = "Community Status") +
    theme_light() +
    labs(title = "Welfare Distribution",
         subtitle = "Disadvantaged (orange) Non-Disadvantaged (teal) over time",
         x = "Year",
         y = "Welfare Value") +
    coord_cartesian(ylim = c(-1000, 200)) +
    # This line sets everything to Times New Roman
...
```{r}
ggplot(t, aes(x = as.factor(Year), y = mean_value, fill = as.factor(soc_vulnerable))) +
    geom_boxplot(outlier.size = 0.5, alpha = 0.8) +
    scale_fill_manual(values = c("0" = "white", "1" = "grey"), 
                      labels = c("Non-DAC", "DAC"), name = "Community Status") +
    theme_light() +
...
ggplot(t, aes(x = island, y = mean_value, fill = factor(env_burden))) +
    ...
    labs(
        x = "Island",
        y = "Average Welfare Loss per ZIP",
        title = "Welfare Loss by Island using EJScreen 90th Environmental Burdens"
    )
...
ggplot(t, aes(x = as.factor(Year), y = mean_value, fill = as.factor(env_burden))) +
    geom_boxplot(outlier.size = 0.5, alpha = 0.8) +
    scale_fill_manual(values = c("0" = "white", "1" = "grey"), 
                      labels = c("Non-DAC", "DAC"), name = "Community Status") +
    theme_light() +
...
summary_table1 <- t %>%
    st_drop_geometry() %>%
    group_by(soc_vulnerable, island) %>%
    summarise(
        variance = var(mean_value), 
        median = median(mean_value), 
        mean = mean(mean_value),
    ...
    mutate(
        soc_vulnerable = ifelse(soc_vulnerable, "Disadvantaged", "Not Disadvantaged")
    )




professional_table <- summary_table %>%
    flextable() %>%
    set_header_labels(
        Indicator = "Indicator",
        island = "Island",
    ...
    colformat_double(j = "variance", digits = 0) %>%
    colformat_double(j = c("median", "mean"), digits = 2) %>%
    merge_v(j = "Indicator") %>%
    theme_booktabs() %>%
    bold(part = "header") %>%
    align(j = c("variance", "median", "mean"), align = "center", part = "all") %>%
    bg(part = "header", bg = "white") %>%
    color(part = "header", color = "black") %>%
    bg(i = seq(2, nrow(summary_table), by = 2), bg = "#F2F2F2", part = "body") %>%
    autofit() %>%
    border_outer(border = fp_border(color = "black", width = 2)) %>%
    border_inner_h(border = fp_border(color = "gray", width = 1)) %>%
    border_inner_v(border = fp_border(color = "gray", width = 1)) %>%
    font(fontname = "Times New Roman", part = "all") %>%  # Add Times New Roman
    fontsize(size = 12, part = "all")  # Standard 12pt font

# View
professional_table

save_as_docx(professional_table, path = "/Users/ashleylowe/welfare_table.docx")
...
This explains why your Total Annual Loss numbers differ.

-   **RUM** Total Surplus grows linearly per trip and is a diaginol.

-   **MDCEV** Total Surplus grows like a "plateau" (it levels off as more and more trips).

This explains the difference in the value of the two
