...
Why is the gradient the way it is? Why does it go downward in the 52-week choice set
...
# ── Step 1: Build clean income lookup — one row per id ────────────────────────
...
# ── Step 2: Site by income — remove income from summarise, join separately ─────
...
# ── Weighted gap by income block ───────────────────────────────────────────────
income_site_gap <- site_by_income %>%
  left_join(site_gap %>% select(parkname, gap, ratio),
            by = "parkname") %>%
  group_by(income_block) %>%
  summarise(
    weighted_gap   = round(sum(gap   * pct_trips / 100, na.rm = TRUE), 2),
    weighted_ratio = round(sum(ratio * pct_trips / 100, na.rm = TRUE), 2),
    n_sites_visited = n_distinct(parkname),
    .groups        = "drop"
  ) %>%
  arrange(income_block)

print(income_site_gap)
